    ...
    try {
        chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            ...
            if (!url.includes('web.whatsapp.com')) {
                var msg = "Please open web.whatsapp.com and connect with your WhatsApp. Then try";
                reset_controls();
                alert1(msg);
                return;
            }
            ...
            function scrollDown() {
                ...
                if (last_elem) {
                    last_elem.scrollIntoView({ behavior: "smooth", block: "start", inline: "nearest" }); 
                }
                return "ok";
            }
            function getItems() {
            ...
            function doit() {
                ...
                chrome.tabs.executeScript({
                    ...
                    if (got)
                        ...
                        no_count++; 
                    lblStatus.innerText = "Got " + nums_all.length.toString() +" numbers. Please wait...";
                    if (no_count > 10) {
                        lblStatus.innerText = "Finished. Got total " + nums_all.length.toString() + " numbers.";
                        exp_now();
                        return;
                    }
                    chrome.tabs.executeScript({
...
function exp_now() {
    if (nums_all.length == 0) {
        alert1("No new numbers available to export");
        return;
    }
    var res = '';
    for (var i = 0; i < nums_all.length; i++) {
        res += nums_all[i];
        if (i < nums_all.length - 1)
            res += "\n";
    } 
